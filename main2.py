...
def main():
    ...
    plt.subplot(4, 1, 1)
    plt.plot(sol.t, sol.y[0], label='Магнит (z)', color='blue')
    plt.plot(sol.t, sol.y[6], label='Шейкер (z)', color='green')
    plt.plot(sol.t, sol.y[2], label='Верхний магнит (z)', color='purple')
    plt.plot(sol.t, sol.y[4], label='Нижний магнит (z)', color='orange')
    plt.plot(
        sol.t,
        [coil.position for _ in sol.t],
        label='Катушка нижняя граница',
        color='red',
    )
    plt.plot(
        sol.t,
        [(coil.position + coil.height) for _ in sol.t],
