...
for μ in range(5, 301, 5): 
    ...
    def get_s(z_top):
        ...
        coil_position = round((z_top+z_bottom)/2 - 0.025, 7)
        coil = Coil(
            turns_count=208,
            thickness=0.01025,
            radius=0.01025,
        ...
        # print(f"Top magnent    | {z_top} м")
        # print(f"Top coil       | {round(coil_position+0.05, 7)} м")
        # print(f"Bottom coil    | {coil_position} м")
        # print(f"Bottom magnent | {z_bottom} м")
        # print("______________________")
    ...
    if __name__ == '__main__':  
        top_magnet_positions = np.arange(0.07, 0.20, 0.01)
        z_values = []
        s_values = []
        ...
        data.to_csv(f'z_s_center_coil_position2_mzs_0.05/csv/s_z__{μ}.csv', index=False)
        ...
        plt.xticks(np.arange(0.07, 0.15, 0.01))
        plt.legend()
        plt.grid(True)
        plt.savefig(f'z_s_center_coil_position2_mzs_0.05/png/s_z__{μ}.png')
